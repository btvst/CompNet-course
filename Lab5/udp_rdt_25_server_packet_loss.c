...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        case 0:
            ...
            // ran = rand()%2;
            // rcv_pkt.sq_no = ran;
            // printf("ran in state 0 of server: %d\n", ran);
            //No packet drop
            if(rcv_pkt.sq_no==0){
            ...
            else{ //Dropped packet 0 from sender to receiver
                printf("Packet 0 from sender dropped ;; Sending ACK1\n");
                // sleep(0.1);
                ack_pkt.sq_no = 1;
                if(sendto(s, &ack_pkt, sizeof(ACK_PKT), 0, (struct sockaddr*)&clntAddr, slen)==-1){
                    die("ack sendto()--server\n");               
        ...
        case 1:
            ...
            // ran = rand()%2;
            // rcv_pkt.sq_no = ran;
            // printf("ran in state 1 of server: %d\n", ran);
            ...
            if(rcv_pkt.sq_no == 1){
                ...
                printf("Packet 1 from sender dropped ;; Sending ACK0\n");
                sleep(0.1);
                ack_pkt.sq_no = 0;
                if(sendto(s, &ack_pkt, sizeof(ACK_PKT), 0, (struct sockaddr*)&clntAddr, slen)==-1){
                    die("ack sendto()--server\n");               

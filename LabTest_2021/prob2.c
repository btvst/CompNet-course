...
* Change READY_TIME macro to alter sleeping time waiting for the peer prcess to bind to its socket

* IMPORTANT - For the program to work correctly, the receiver program must be run before the sender program.

* If only want to receive (no send), put <input filename> in cmd as 'NULL'

* If only want to send (no receive), put <output filename> in cmd as 'NULL
...
//sender variables
int peer1_sockfd, i, ran, recv_flag, peer1_sending_socket;
struct sockaddr_in peer2_addr, peer1_addr;
...
int main(int argc, char *argv[]){
    ...
    if((peer1_sockfd = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) == -1){
    ...
    if(bind(peer1_sockfd, (struct sockaddr*)&peer1_addr, slen) == -1){
        die_with_error("bind() error in PEER1");
    }

    printf("Bind complete.\n"); //Sleeping for %d seconds to wait for receiver to get ready\n", READY_TIME);
    // sleep(READY_TIME);
    // printf("Woken up from sleep\n");

    if((pid = fork())<0){ //fork to simultaneously send and receive
        die_with_error("fork() in PEER1");
    }
    else if(pid == 0){ //sending file code
        if(strcmp(argv[1], nl) != 0){
            ...
            while(curr_ind < no_of_packets){
                ...
                case 0:
                    ...
                    break;
                case 1:
                    if(recvfrom(peer1_sending_socket, &recv_packet, sizeof(packet), 0, (struct sockaddr*)&peer2_addr, &slen) == -1){
    ...
    else{//receiving is done by parent
            ...
            while(1){
                ...
                case 0:
                    ...
                    if(packet_drop_counter == 10){
                        ...
                        if((recv_len = recvfrom(peer1_sockfd, &recv_packet, sizeof(packet), 0, (struct sockaddr*)&peer2_addr, &slen)) == -1){
                        ...
                        if(sendto(peer1_sockfd, &send_packet, sizeof(packet), 0, (struct sockaddr*)&peer2_addr, slen) <= 0){
